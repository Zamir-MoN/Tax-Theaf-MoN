...
export default {
  ...
  async execute(interaction) {
    ...
    // Split games into chunks of 25 because Discord limits 25 options per dropdown
    const chunkSize = 25;
    const actionRows = [];

    for (let i = 0; i < availableGames.length; i += chunkSize) {
        const chunk = availableGames.slice(i, i + chunkSize);
        
        const select = new StringSelectMenuBuilder()
            .setCustomId(`select_game_acc_${i}`)
            .setPlaceholder(`Select a game (Part ${Math.floor(i / chunkSize) + 1})`)
            .addOptions(
                chunk.map(game => 
